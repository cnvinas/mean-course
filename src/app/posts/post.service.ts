...
export class PostsService {
            ...
            .pipe(
                map(postData => {
                    return postData.posts.map(post => {
                        return { title: post.title, content: post.content, id: post._id, image: post.image  }
                    });
                })
            )
    ...
    addPosts(title: string, content: string, image: File) {
        const post: Post = { id: null, title, content, image };
        this.http
            .post<{ message: string, postId: string, image: File }>('http://localhost:3000/api/posts', post)
            .subscribe((res) => {
